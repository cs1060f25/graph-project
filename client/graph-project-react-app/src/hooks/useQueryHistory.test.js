...
describe('useQueryHistory', () => {
  ...
  test('addToHistory updates local state with timestamp from API', async () => {
    ...
    userApi.getQueryHistory.mockResolvedValue([]);
    userApi.addQueryHistory.mockResolvedValue(mockNewQuery);
  ...
  test('formatTimestamp handles valid timestamp correctly', () => {
  ...
  test('formatTimestamp handles invalid timestamp (edge case)', () => {
    const { result } = renderHook(() => useQueryHistory(false));
  ...
  test('formatTimestamp handles string timestamp (ISO format)', () => {
    const { result } = renderHook(() => useQueryHistory(false));
  ...
  test('history is prepended in chronological order', async () => {
    const existingQuery = {
      id: 'old-id',
      query: 'old query',
      type: 'keyword',
    ...
    userApi.getQueryHistory.mockResolvedValue([existingQuery]);
    userApi.addQueryHistory.mockResolvedValue(newQuery);
  ...
  test('clearHistory empties the history array', async () => {
    ...
    userApi.clearQueryHistory.mockResolvedValue();
