...
export class AgentService {
    ...
    async generateContentFromVertexAI(query) {
    ...
    async calculateSemanticSimilarity(a, b) {
    ...
    async rerankResults(query, results) {
        ...
        const similarityFns = results.map((paper) => {
    ...
    async query(query) {
        try {
            ...
            const requestFns = [
                ...arxiv_queries.map((q) => searchArxiv(q.query, 10, 0, q.mode || "")),
                ...openalex_queries.map((q) => searchOpenalex(q.query, 10)),
                ...core_queries.map((q) => searchCore(q.query, 10, q.mode || ""))
            ];
