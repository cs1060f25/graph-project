...
router.get('/papers', async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.uid) {
...
router.post('/papers', async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.uid) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
...
router.patch('/papers/:paperId', async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.uid) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
...
router.delete('/papers/:paperId', async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.uid) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
