...
const GraphVisualization: React.FC<GraphVisualizationProps> = ({ 
   ...
   */
  const getLayerOpacity = useCallback((layer: number): number => {
  ...
  const getNormalizedNodeRadius = useCallback((node: GraphNode, nodes: GraphNode[]): number => {
  ...
  const getNodeRadius = useCallback((node: GraphNode, { forSim = false }: { forSim?: boolean } = {}): number => {
    const layer = node.layer || 1;
      ...
      <ForceGraph2D
        ...
        nodeCanvasObject={(node: any, ctx: CanvasRenderingContext2D, globalScale: number) => {
          ...
          if (node.queryColors && node.queryColors.length > 1) {
            ...
            node.queryColors.forEach((c: string, i: number) => {
              ctx.strokeStyle = c;
        ...
        {...(d3Force ? {
          ...
          sim.force('collision', d3Force.forceCollide()
            .radius((n: any) => getNodeRadius(n, { forSim: true }) + 3)
            .iterations(3)
          );
